...
The user interacts with FinChat to understand the annual report.
Answer the user's questions, understanding synonymous words related to the document.
Be friendly and cheerful but do not use emoticons.
Address the document in your responses.
...
def upload_file():
    ...
    try:
        ...
        def embed(document):
            try:
                ...
                embeddings = genai.embed_content(model='models/text-embedding-004',content=document.page_content,task_type='retrieval_document')
                r= {
                    'id': hash_id,
        ...
        for vector_chunks in chunks(vectors, batch_size=1000):
            pinecone_index.upsert(vector_chunks, namespace=convert_to_ascii(file_key))
    ...
    #CHATS TABLE INSERT
    try:
        new_chat = Chats(pdf_name=file.filename, pdf_url=s3_file_url, username=current_user.username, file_key=file_key)
        db.session.add(new_chat)
        db.session.commit()
        return jsonify({'message': 'Files uploaded and processed successfully','pdf_url': s3_file_url}), 200
    except Exception as e:
...
def get_chats():
    try:
        ...
        chats = [{'pdf_name': chat.pdf_name, 'pdf_url': chat.pdf_url,'id':chat.chat_id} for chat in user_chats]
...
def init_chat():
    ...
    try:
        ...
        app.config['CHATS'][chat_id] = {'chat': chat, 'file_key': chat_details.file_key}
...
def query_gemini():
    ...
    try:
        ...
        file_key=chat_session['file_key']
        ...
        return jsonify({'response': response.text,'context':context}), 200
    except Exception as e:
...
def get_messages(chat_id):
    try:
        ...
        return jsonify([{'id':message.id,'content':message.content,'role':message.role.value} for message in messages]), 200
    except Exception as e:
...
def delete_chat(chat_id):
    try:
        ...
        # Delete from S3
        s3.delete_object(Bucket=os.getenv('PUBLIC_S3_BUCKET_NAME'), Key=chat.file_key)

        # Delete from Pinecone
        pinecone_index.delete(delete_all=True,namespace=convert_to_ascii(chat.file_key))
