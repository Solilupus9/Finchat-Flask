...
function ChatPage() {
	...
	const [activatedPageNumber, setActivatedPageNumber] = useState(0);

	useEffect(() => {
		console.log('Activated page number:', activatedPageNumber);
	}, [activatedPageNumber]);
	...
	useEffect(() => {
		async function fetchChats() {
			try {
				...
				setChats(data);
				if (data.length > 0) {
					setSelectedPdfUrl(data.at(-1).pdf_urls[0]);
	...
	return (
		<div className={`${styles.chatPage} d-flex flex-row`}>
			...
			<PDFViewer pdfUrl={selectedPdfUrl} context={context} pageNumber={activatedPageNumber} />
			<messageContext.Provider value={{messages, isLoading,pageNumbers,setActivatedPageNumber}}>
				<div className="flex-grow-1">
					<Chat chatId={selectedChatId} onNewMessage={fetchMessages} setContext={setContext} setPageNumbers={setPageNumbers}/>
				</div>
