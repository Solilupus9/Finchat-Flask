...
function MessageList() {
    ...
    return (
        <div className="flex-grow-1 h-100 border border-dark-subtle mx-2 rounded-3" style={{ maxHeight: '80vh', overflowY: 'auto' }}>
                ...
                <div>
                    {messages.messages.map((message, index) => (
                        <div key={index}
                            ...
                            <div>
                                {message.content && message.role === 'model' && message.content.includes("Highcharts.chart('container',")? (
                                ...
                                {index === messages.messages.length - 1 && messages.pageNumbers.length > 0 && (
                                    <span className="ms-2">
                                        {messages.pageNumbers.map((pageNumber) => (
                                            <span
                                                key={pageNumber}
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                    messages.setActivatedPageNumber(pageNumber);
                                                }}
                                                style={{ cursor: 'pointer', textDecoration: 'underline', margin: '0 5px' }}
                                            >
                                                {pageNumber}
                                            </span>
                                        ))}
